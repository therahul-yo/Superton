...
5. **No silent gaps over ~200 ms.** Any blocking work — retrieval, model
   warm-up, first-token wait — needs a visible animation. A still
   terminal reads as a hang.
6. **First 10 minutes are sacred.** Anything that delays the user's first
...
**Moment**: user runs `superton` (no args) for the first time and asks
their first question.

**What's already good** (after this PR):
- Retrieval phase shows a `searching palace…` spinner so the gap between
  Enter and the Miniton header isn't dead air.
- `ui.stream_answer` shows a Rich `Spinner("dots") + thinking…` during
  time-to-first-token, animated on a daemon thread so it keeps ticking
  while `model.generate()` blocks on the first network round-trip.
- Once tokens flow, the spinner is dropped and the existing muted-cursor
  streaming view takes over.
...
### Phase 1 — this PR
- ✅ Tri-color install palette (orange / yellow / red).
- ✅ Miniton loading animation: retrieval spinner + thinking spinner
  during time-to-first-token in `ui.stream_answer`.
- ✅ This plan doc.
