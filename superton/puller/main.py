...
def _as_page(r: WorkerResult) -> Page | None:
    if not r.ok or not r.url or not (r.content or "").strip():
        return None
    return Page(url=r.url, title=r.title or "", markdown=frontmatter(r.title or r.url, r.url) + (r.content or ""))

async def pull_url(url: str, *, render_js: bool = False, timeout: float = 30.0) -> Page | None:
    async with httpx.AsyncClient(headers=get_headers(), follow_redirects=True, timeout=timeout) as client:
        result = await fetch_and_extract(url, client=client, use_browser=render_js)
    return _as_page(result)

async def pull_site(url: str, *, max_pages: int = 500, render_js: bool = False, concurrency: int | None = None, timeout: float = 30.0) -> AsyncIterator[Page]:
    ...
    if render_js:
        try:
            async with httpx.AsyncClient(headers=get_headers(), follow_redirects=True, timeout=timeout) as c:
                r = await c.get(url)
    ...
    async with WorkerPool(concurrency=concurrency, use_browser=needs_browser, timeout=timeout) as pool:
        ...
        try:
            ...
            st.cancel()
            with suppress(asyncio.CancelledError): await st
            if not task.done():
                task.cancel()
