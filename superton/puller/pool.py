...
class WorkerPool:
    def __init__(self, concurrency: int = 8, *, use_browser: bool = False, timeout: float = 30.0):
    ...
    async def __aenter__(self) -> "WorkerPool":
        self._client = httpx.AsyncClient(headers=get_headers(), follow_redirects=True, timeout=self.timeout)
        return self
    ...
    async def pull_all(self, urls: list[str], *, on_start: OnStart | None = None, on_done: OnDone | None = None) -> list[WorkerResult]:
        ...
        async def _task(idx: int, url: str) -> None:
            async with sem:
                if on_start:
                    try: on_start(idx, url)
                    except Exception: pass
                assert self._client
                res = await fetch_and_extract(url, client=self._client, use_browser=self.use_browser)
                results[idx] = res
                if on_done:
                    try:
                        r = on_done(res, idx)
